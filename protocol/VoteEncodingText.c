...
// format = <MAGIC> <VOTESTR|INQSTR> <RES|candidate ID> <COUNT>
...
bool Decode(uint8_t *inBuf, const size_t mSize, VoteInfo *v) {
    ...
    // get the frist token separated by DELIM
    ...
    // check if "v" || "i"
    if(strcmp(token, VOTESTR) == 0) v->isInquiry = false;
    else if(strcmp(token, INQSTR) == 0) v->isInquiry = true;
    else return false;
